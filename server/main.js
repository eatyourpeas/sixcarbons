...
Meteor.startup(() => {
  ...
  Meteor.methods({
        getMilks: function () {
          var arrayOfAllMilks = [];
          arrayOfAllMilks = convertArrayOfMilksToArrayOfMilkObjects();
          var newArray = _.sortBy(arrayOfAllMilks, 'name');
          return newArray;
        }
...
function convertArrayOfMilksToArrayOfMilkObjects(){

  var arrayOfMilkObjects = [];
  ...
  for (var i = 0; i < allmilks.length; i=i+2) {
    var thisMilk = new Milk();
    thisMilk.name = allmilks[i];
    thisMilk.grams = allmilks[i+1];
    arrayOfMilkObjects.push(thisMilk);
  }
  return arrayOfMilkObjects;
